...
impl RotationUniform {
    fn new() -> Self {
        ...
        Self{matrix: cgmath::Matrix4::identity().into()}
    }

    fn update_rotation(&mut self, rotation: f32) {
        self.matrix[1][1] = rotation.cos();
        self.matrix[1][2] = -rotation.sin();
        self.matrix[2][1] = rotation.sin();
        self.matrix[2][2] = rotation.cos();
    }
}
...
impl InstanceRaw {
    fn desc() -> wgpu::VertexBufferLayout<'static> {
        ...
        wgpu::VertexBufferLayout {
            ...
            attributes: &[
                // One vertex attribute for each vector (Mat4)
                // To be re-assembled in shader
...
impl<'a> State<'a> {
        ...
        match event {
            WindowEvent::CursorMoved{device_id: _, position} => {
                self.color = wgpu::Color{
                    ...
                    b: (position.x / self.config.width as f64) - (position.y / self.config.height as f64),
                    a: 1.0
                }
            }
    ...
    pub fn update(&mut self) {
        ...
        let subsecs = since_the_epoch.subsec_millis();
        let rotation = (subsecs as f32 * 0.01).sin();

        self.rotation_uniform.update_rotation(rotation);
        self.queue.write_buffer(&self.rotation_buffer, 0, bytemuck::cast_slice(&[self.rotation_uniform]));
...
pub async fn run() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_arch = "wasm32")] {
            // Send panics to javascript console
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        web_sys::window()
            ...
            .and_then(|doc| {
                let dst = doc.get_element_by_id("portfolio")?;
